...
public class BookSelectNo extends AppCompatActivity {
    private EditText bookno;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.book_select_no);
        bookno = (EditText) findViewById(R.id.editText);
    }
    public void select(View v) {
        ListView lv = (ListView) findViewById(R.id.lv);
        BookControl bookcontrol = new BookControl(BookSelectNo.this);
        String book_no = bookno.getText().toString().trim();//输入的字段
        Book book[] = bookcontrol.QueryOnByNo(book_no);
        //获取到集合数据
